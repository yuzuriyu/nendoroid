...
const Cart = () => {
    ...
    return (
        <div className="pt-32 md:pb-32 w-11/12 m-auto md:w-10/12">
            ...
            <div className="w-full md:w-1/2">
                {cart.map((item => <div className="flex border-b-2 py-2 items-center">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-gray-500 dark:text-white">Qty: {quantities[item.id]} </p>
                        <div className="flex justify-between py-3">
                            <div className="flex items-center">
                                <img src={minus} alt="minus" onClick={() => subtract(item.id)} className="cursor-pointer hover:scale-110 transition-transform ease-in-out"/>
                                <p className="mx-2 dark:text-white">{quantities[item.id]}</p>
                                <img src={plus} alt="plus" onClick={() => add(item.id)} className="cursor-pointer hover:scale-110 transition-transform ease-in-out"/>
                            </div>
            ...
            {cart.length !== 0 && <div className="flex py-4 md:w-1/2">
                <div className="mr-4 w-1/4">
                    <p className="text-gray-500 text-sm">Total</p>
                    <h1 className="text-xl text-orange-400 font-bold">¥{total}</h1>
                </div>
                <button className="bg-black flex-1 text-white rounded-lg md:w-2/3 dark:bg-orange-400">Proceed to checkout</button>
            </div>}
        </div>
    )
}
